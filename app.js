...
function createOptExPdf(r,p){
  ...
  const wrap=(t,xx,yy,maxW,size=20,weight="400",color="#344054",lh=28,max=5)=>{
  ...
  const box=(xx,yy,w,h,stroke="#d0d5dd",fill="#fff")=>{x.fillStyle=fill;x.strokeStyle=stroke;x.lineWidth=1.5;x.beginPath();x.roundRect(xx,yy,w,h,16);x.fill();x.stroke()};

  text("OptEx Nexus",M,90,46,"700","#101828");
  text(`${p.code}  •  ${r.title||"OptEx Nexus"}`,M,132,22,"400","#667085");
  ...
  text("Issue redoutée",M,190,18,"700","#475467");
  box(M,210,W-2*M,110,"#d0d5dd","#f8fafc");
  wrap(r.outcome||"—",M+24,250,W-2*M-48,24,"600","#101828",32,2);

  const colW=(W-2*M-18)/2;
  box(M,350,colW,760,"#d9a1aa","#fff");
  box(M+colW+18,350,colW,760,"#9ca6e8","#fff");
  text("Prédicteurs",M+24,392,24,"700","#a52337");
  text("Inhibiteurs",M+colW+42,392,24,"700","#2838b9");

  let y1=435;
  for(const[key,label]of PREDICTOR_CATEGORIES){
    const items=Array.isArray(r.predictors?.[key])?r.predictors[key]:[];
    if(items.length){
      text(label,M+24,y1,18,"700","#475467");y1+=28;
      for(const item of items){
        y1+=wrap("• "+item.text,M+24,y1,colW-48,17,"400","#344054",23,2)+4;
        if(item.type||item.maximal){
          text([item.type,item.maximal?"Prédiction maximale":""].filter(Boolean).join(" • "),M+42,y1,14,"400","#98a2b3");
          y1+=22;
        }
      }
      y1+=10;
  ...
  if(r.principalCS){
    text("Principal SC",M+24,y1,18,"700","#475467");y1+=28;
    wrap(r.principalCS,M+24,y1,colW-48,18,"400","#344054",24,3);
  }
  ...
  for(const[label,val]of inhibitors){
    if(val){
      text(label,M+colW+42,y2,18,"700","#475467");y2+=28;
      y2+=wrap(val,M+colW+42,y2,colW-48,18,"400","#344054",24,3)+18;
    }
  }

  box(M,1140,W-2*M,430,"#344054","#fff");
  text("Exposition optimale",M+24,1184,25,"700","#101828");
  text("Exercice proposé",M+24,1232,18,"700","#475467");
  wrap(r.optimalExposure||"—",M+24,1264,W-2*M-48,20,"500","#172033",28,5);
  text("Durée ou critère d’arrêt",M+24,1432,18,"700","#475467");
  wrap(r.duration||"—",M+24,1464,W-2*M-48,20,"400","#344054",28,3);

  wrap("Craske, M. G., Treanor, M., Zbozinek, T. D., & Vervliet, B. (2022). Optimizing exposure therapy with an inhibitory retrieval approach and the OptEx Nexus. Behaviour Research and Therapy, 152, 104069.",M,H-92,W-2*M,14,"400","#98a2b3",19,2);
...
function createHierarchyPdf(r,p){
  ...
  text("Synthèse générée avec ExpoClinique",W-M,H-38,15,"400","#98a2b3","right");
...
function createScriptPdf(r,p){
  const W=1240,H=1754,M=74;const c=document.createElement("canvas");c.width=W;c.height=H;const x=c.getContext("2d");x.fillStyle="#fff";x.fillRect(0,0,W,H);
  const text=(t,xx,yy,size=22,weight="400",color="#172033",align="left")=>{x.fillStyle=color;x.font=`${weight} ${size}px Arial`;x.textAlign=align;x.textBaseline="alphabetic";x.fillText(String(t??""),xx,yy)};
  const wrap=(t,xx,yy,maxW,size=20,weight="400",color="#344054",lh=27,maxLines=12)=>{x.font=`${weight} ${size}px Arial`;x.fillStyle=color;x.textAlign="left";const words=String(t??"").split(/\s+/).filter(Boolean),lines=[];let line="";for(const word of words){const test=line?line+" "+word:word;if(x.measureText(test).width>maxW&&line){lines.push(line);line=word;if(lines.length===maxLines-1)break}else line=test}if(line&&lines.length<maxLines)lines.push(line);lines.forEach((lineText,index)=>x.fillText(lineText,xx,yy+index*lh));return lines.length*lh};
  const section=(title,content,y,maxLines)=>{text(title,M,y,21,"700","#101828");y+=34;const used=wrap(content||"—",M,y,W-2*M,19,"400","#344054",26,maxLines);return y+Math.max(used,26)+28};
  const catastrophe=r.type==="catastrophe";text(catastrophe?"Scénario catastrophe":"Script traumatique",M,92,44,"700","#101828");text(`${p.code}  •  ${r.eventName}`,M,132,22,"400","#667085");
  if(getPsychologistName())text(`Professionnel : ${getPsychologistName()}`,W-M,132,18,"400","#667085","right");
  text(`Date d’édition : ${formattedEditionDate()}`,W-M,156,15,"400","#98a2b3","right");x.strokeStyle="#d0d5dd";x.beginPath();x.moveTo(M,162);x.lineTo(W-M,162);x.stroke();let y=210;
  if(catastrophe){y=section("Déclencheur et contexte initial",r.catastropheTrigger,y,8);y=section("Début de la catastrophe",r.catastropheBeginning,y,8);y=section("Le pire moment",r.catastropheWorst,y,14);y=section("Conséquences durables",r.catastropheConsequences,y,10);y=section("Fin du scénario",r.catastropheEnding,y,8)}else{y=section("Avant l’événement — en sécurité",r.before,y,8);y=section("Début de l’événement",r.beginning,y,8);y=section("Hot spot",r.hotspot,y,14);y=section("Fin de l’événement",r.end,y,8);y=section("Une fois l’événement terminé — de nouveau en sécurité",r.after,y,8)}
  text("Document généré avec Exposition",W-M,H-38,15,"400","#98a2b3","right");return imagesToPdf([c.toDataURL("image/jpeg",0.94)],W,H)}
