...
function createOptExPdf(r,p){
  ...
  function wrapLines(t,maxW,size=18,weight="400"){
    x.font=`${weight} ${size}px Arial`;
    const words=String(t??"").split(/\s+/).filter(Boolean);
    const lines=[];
    let line="";
  ...
  function box(xx,yy,w,h,stroke="#d0d5dd",fill="#fff"){
    ...
    x.lineWidth=1.5;
    x.beginPath();
    x.roundRect(xx,yy,w,h,16);
    x.fill();
    x.stroke();
  }

  function addPage(title="OptEx Nexus"){
    ...
    text(title,M,82,38,"700","#101828");
    text(`${p.code}  •  ${r.title||"OptEx Nexus"}`,M,122,21,"400","#667085");

    if(getPsychologistName()){
      text(`Professionnel : ${getPsychologistName()}`,W-M,122,18,"400","#667085","right");
    }
    if(typeof formattedEditionDate==="function"){
      text(`Date d’édition : ${formattedEditionDate()}`,W-M,148,15,"400","#98a2b3","right");
    }
  ...
  function ensureSpace(required,title="OptEx Nexus — suite"){
    if(y+required>H-105){
      addPage(title);
    }
  }

  function drawParagraph(label,value,options={}){
    if(value===null||value===undefined||value==="")return;
    const size=options.size||18;
    const lineH=options.lineH||25;
    const lines=wrapLines(value,W-2*M-40,size,"400");
    const needed=38+lines.length*lineH+22;

    ensureSpace(needed);

    text(label,M,y,18,"700","#475467");
    y+=30;

    for(const line of lines){
      text(line,M,y,size,"400","#344054");
      y+=lineH;
    }
    y+=20;
  }

  function drawPredictorGroup(group){
    const headerSpace=36;
    ensureSpace(headerSpace+50);

    text(group.label,M,y,19,"700","#a52337");
    y+=30;

    for(const item of group.items){
      const mainLines=wrapLines(`• ${item.text}`,W-2*M-40,17,"400");
      const meta=[item.type,item.maximal?"Prédiction maximale":""].filter(Boolean).join(" • ");
      const needed=mainLines.length*23+(meta?22:0)+14;

      ensureSpace(needed);

      for(const line of mainLines){
        text(line,M,y,17,"400","#344054");
        y+=23;
      }

      if(meta){
        text(meta,M+20,y,14,"400","#98a2b3");
        y+=20;
      }
  ...
  function drawInhibitor(label,value){
    const lines=wrapLines(value,W-2*M-40,17,"400");
    const needed=36+lines.length*23+18;

    ensureSpace(needed);

    text(label,M,y,19,"700","#2838b9");
    y+=30;
  ...
  // Issue redoutée
  ensureSpace(150);
  text("Issue redoutée",M,y,18,"700","#475467");
  y+=22;
  const outcomeLines=wrapLines(r.outcome||"—",W-2*M-48,22,"600");
  const outcomeHeight=Math.max(90,outcomeLines.length*31+34);
  box(M,y,W-2*M,outcomeHeight,"#d0d5dd","#f8fafc");
  let outcomeY=y+38;
  for(const line of outcomeLines){
    text(line,M+24,outcomeY,22,"600","#101828");
    outcomeY+=31;
  }
  y+=outcomeHeight+34;
  ...
  // Prédicteurs
  ensureSpace(48);
  text("1. Prédicteurs de l’issue redoutée",M,y,24,"700","#101828");
  y+=42;

  if(!predictors.length){
    drawParagraph("Prédicteurs","Aucun prédicteur renseigné.");
  }else{
    for(const group of predictors){
      drawPredictorGroup(group);
    }
  }

  if(r.principalCS){
    drawParagraph("Principal stimulus conditionnel",r.principalCS,{size:18});
  }

  // Inhibiteurs
  ensureSpace(52);
  text("2. Inhibiteurs",M,y,24,"700","#101828");
  y+=42;

  if(!inhibitors.length){
    drawParagraph("Inhibiteurs","Aucun inhibiteur renseigné.");
  }else{
    for(const[label,value]of inhibitors){
      drawInhibitor(label,value);
    }
  }

  // Exposition optimale
  ensureSpace(60);
  text("3. Exposition optimale",M,y,24,"700","#101828");
  y+=42;

  drawParagraph("Exercice proposé",r.optimalExposure||"—",{size:19,lineH:27});
  drawParagraph("Durée ou critère d’arrêt",r.duration||"—",{size:18,lineH:25});

  // Référence scientifique
  const reference="Craske, M. G., Treanor, M., Zbozinek, T. D., & Vervliet, B. (2022). Optimizing exposure therapy with an inhibitory retrieval approach and the OptEx Nexus. Behaviour Research and Therapy, 152, 104069.";
  const refLines=wrapLines(reference,W-2*M,14,"400");
  ensureSpace(refLines.length*19+48);

  text("Référence",M,y,15,"700","#667085");
  y+=24;
  for(const line of refLines){
    text(line,M,y,14,"400","#98a2b3");
    y+=19;
  }

  text("Synthèse générée avec Exposition",W-M,H-36,15,"400","#98a2b3","right");
