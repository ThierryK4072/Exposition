...
let appMode=localStorage.getItem("expositionAppMode")==="patient"?"patient":"professional";
const PATIENT_MODE_CODE="__PATIENT_MODE__";
...
function loadBrandLogo(){
  return new Promise(resolve=>{
    ...
    image.onload=()=>{brandLogoImage=image;resolve()};
    image.onerror=()=>resolve();
    image.src="brand-logo.png";
  });
}

function drawPdfBrandLogo(ctx,x,y,width=245){
  const iconSize=Math.max(34,Math.min(54,width*.19));
  const textX=x+iconSize+12;
  const centerY=y+iconSize/2;

  ctx.save();

  // Icône : carré arrondi bleu et courbe décroissante
  ctx.strokeStyle="#2457c5";
  ctx.lineWidth=Math.max(2.2,iconSize*.055);
  ctx.beginPath();
  ctx.roundRect(x,y,iconSize,iconSize,iconSize*.22);
  ctx.stroke();

  const pts=[
    [x+iconSize*.22,y+iconSize*.30],
    [x+iconSize*.43,y+iconSize*.58],
    [x+iconSize*.64,y+iconSize*.42],
    [x+iconSize*.82,y+iconSize*.72]
  ];

  ctx.strokeStyle="#2457c5";
  ctx.lineWidth=Math.max(2.5,iconSize*.065);
  ctx.lineCap="round";
  ctx.lineJoin="round";
  ctx.beginPath();
  ...
  for(const [px,py] of pts){
    ctx.beginPath();
    ctx.arc(px,py,Math.max(2.8,iconSize*.075),0,Math.PI*2);
    ctx.fill();
  }

  // Nom du site
  const fontSize=Math.max(14,Math.min(22,width*.085));
  ctx.textBaseline="middle";
  ctx.textAlign="left";
  ctx.font=`400 ${fontSize}px Arial`;
  ctx.fillStyle="#101828";
  ctx.fillText("therapie-",textX,centerY);

  const firstWidth=ctx.measureText("therapie-").width;
  ctx.fillStyle="#2457c5";
  ctx.fillText("exposition",textX+firstWidth,centerY);

  const secondWidth=ctx.measureText("exposition").width;
  ctx.fillStyle="#98a2b3";
  ctx.fillText(".com",textX+firstWidth+secondWidth,centerY);
...
async function getPatientModeProfile(){
  const patients=await all("patients");
  let profile=patients.find(p=>p.code===PATIENT_MODE_CODE);
  if(profile)return profile;
  profile={code:PATIENT_MODE_CODE,notes:"Espace local utilisé par la version patient.",createdAt:new Date().toISOString(),systemProfile:true};
  profile.id=await add("patients",profile);
  return profile;
}
function isPatientMode(){return appMode==="patient"}
async function setAppMode(mode,{navigate=true}={}){
  appMode=mode==="patient"?"patient":"professional";
  localStorage.setItem("expositionAppMode",appMode);
  document.body.classList.toggle("patient-mode",isPatientMode());
  const professionalBtn=document.getElementById("professionalModeBtn");
  const patientBtn=document.getElementById("patientModeBtn");
  if(professionalBtn){professionalBtn.classList.toggle("active",!isPatientMode());professionalBtn.setAttribute("aria-pressed",String(!isPatientMode()))}
  if(patientBtn){patientBtn.classList.toggle("active",isPatientMode());patientBtn.setAttribute("aria-pressed",String(isPatientMode()))}
  const subtitle=document.getElementById("appModeSubtitle");
  if(subtitle)subtitle.textContent=isPatientMode()?"Réalisation et suivi de mes exercices d’exposition":"Suivi des séances d’exposition";
  document.querySelectorAll('[data-mode="professional"]').forEach(el=>el.classList.toggle("mode-hidden",isPatientMode()));
  document.getElementById("sessionPatientField")?.classList.toggle("mode-hidden",isPatientMode());
  document.getElementById("historyPatientFilterField")?.classList.toggle("mode-hidden",isPatientMode());
  document.getElementById("patientModeNotice")?.classList.toggle("hidden",!isPatientMode());
  document.getElementById("patientHistoryNotice")?.classList.toggle("hidden",!isPatientMode());
  if(isPatientMode()){
    const profile=await getPatientModeProfile();
    if(document.getElementById("spatient")){spatient.required=false;spatient.value=String(profile.id)}
    if(document.getElementById("hpatient"))hpatient.value=String(profile.id);
  }else if(document.getElementById("spatient")){spatient.required=true}
  await refresh();
  if(navigate)show(isPatientMode()?"new":"home");
  else{const activeView=document.querySelector(".view.active")?.id;if(isPatientMode()&&!['new','history','about'].includes(activeView||''))show('new')}
}
function bindModeSwitch(){
  document.getElementById("professionalModeBtn")?.addEventListener("click",()=>setAppMode("professional"));
  document.getElementById("patientModeBtn")?.addEventListener("click",()=>setAppMode("patient"));
}

async function init(){
  loadBrandLogo();
  db=await openDB();
  bindNav();bind();bindModeSwitch();buildMetrics();buildSelects();sliders();
  ...
  buildPredictorCategories();addDefaultHierarchyItems();await setAppMode(appMode,{navigate:false});setupPatientSearches();applyScriptType();loadPsychologistName();setupModernRanges();
  if("serviceWorker"in navigator)navigator.serviceWorker.register("service-worker.js").catch(()=>{});
}

function bindNav(){document.querySelectorAll("[data-v]").forEach(b=>b.onclick=()=>show(b.dataset.v))}
function show(id){if(isPatientMode()&&!['new','history','about'].includes(id))id='new';document.querySelectorAll('.view').forEach(v=>v.classList.toggle('active',v.id===id));document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.v===id));scrollTo(0,0)}
function bind(){
...
async function startSession(e){
  ...
  let pid;
  if(isPatientMode()){const patientProfile=await getPatientModeProfile();pid=patientProfile.id;spatient.value=String(pid)}
  else{pid=Number(document.getElementById("spatient").value);if(!pid){toast("Sélectionnez un patient.");return}}
...
goal:goal.value.trim(),safetyGoal:safetygoal.value.trim(),date:new Date().toISOString(),status:"active"};
  ...
  const p=await rq(st("patients").get(pid));livetitle.textContent=s.title;livesub.textContent=isPatientMode()?s.target:`${p.code} — ${s.target}`;
...
async function refresh(){
  const allPatients=(await all("patients")).sort((a,b)=>a.code.localeCompare(b.code));
  const patientModeProfile=allPatients.find(p=>p.code===PATIENT_MODE_CODE);
  const ps=allPatients.filter(p=>p.code!==PATIENT_MODE_CODE);
  const allSessions=await all("sessions");
  const professionalSessions=patientModeProfile?allSessions.filter(s=>s.patientId!==patientModeProfile.id):allSessions;
  pc.textContent=ps.length;sc.textContent=professionalSessions.length;window.__patientsCache=ps;renderPatientList();
  ...
  if(patientModeProfile){for(const s of[spatient,hpatient]){const o=document.createElement("option");o.value=patientModeProfile.id;o.textContent="Mon espace";o.dataset.patientMode="1";s.appendChild(o)}if(isPatientMode()){spatient.value=String(patientModeProfile.id);hpatient.value=String(patientModeProfile.id)}}
...
function setupPatientSearches(){
  ...
  for(const id of selectorIds){
    if(isPatientMode()&&["spatient","hpatient"].includes(id))continue;
    const select=document.getElementById(id);
...
async function renderSessions(){
  const patientModeProfile=(await all("patients")).find(p=>p.code===PATIENT_MODE_CODE);
  let pid=+hpatient.value||0;
  if(isPatientMode())pid=patientModeProfile?.id||0;
  const ss=(await all("sessions"))
    .filter(s=>{if(isPatientMode())return Boolean(pid)&&s.patientId===pid;if(patientModeProfile&&s.patientId===patientModeProfile.id)return false;return !pid||s.patientId===pid})
  ...
  for(const s of ss){
    ...
    b.innerHTML=isPatientMode()?`<b>${esc(s.title)}</b><small>${fmtDate(s.date)} — ${s.status==="completed"?"Terminée":"En cours"}</small>`:`<b>${esc(s.title)}</b><div>${esc(pm[s.patientId]?.code||"")}</div><small>${fmtDate(s.date)} — ${s.status==="completed"?"Terminée":"En cours"}</small>`;
...
async function renderDetail(){
  ...
  hempty.classList.add("hidden");hdetail.classList.remove("hidden");htitle.textContent=isPatientMode()?s.title:`${s.title} — ${p?.code||""}`;
...
async function createSessionPdf(s,p,ms){
  ...
  function newPage(title=true){
    if(canvas){drawPdfBrandLogo(ctx,M,H-70,250);pages.push(canvas.toDataURL("image/jpeg",0.94));}
    ...
    if(title){
      ...
      if(!isPatientMode()&&getPsychologistName())text(`Professionnel : ${getPsychologistName()}`,W-M,y+58,18,"400","#667085","right");
  ...
  drawPdfBrandLogo(ctx,M,H-70,250);
...
async function exportOptExPdf(){
  ...
  try{
    ...
    console.error("Erreur PDF OptEx :",error);
    toast("Impossible de générer le PDF OptEx.");
  }
}
...
function createOptExPdf(r,p){
  ...
  function footer(){drawPdfBrandLogo(x,M,H-68,245);text("",W-M,H-34,14,"400","#98a2b3","right")}
  ...
  function page(first=false){
    ...
    if(!isPatientMode()&&getPsychologistName())text(`Professionnel : ${getPsychologistName()}`,W-M,110,17,"400","#667085","right");
...
async function exportHierarchyPdf(){
  ...
  try{
    ...
    console.error("Erreur PDF hiérarchie :",error);
    toast("Impossible de générer le PDF de la hiérarchie.");
  }
}
...
function createHierarchyPdf(r,p){
  ...
  if(!isPatientMode()&&getPsychologistName())text(`Professionnel : ${getPsychologistName()}`,W-M,132,18,"400","#667085","right");
...
async function exportScriptPdf(){
  ...
  try{
    ...
    console.error("Erreur PDF script :",error);
    toast("Impossible de générer le PDF du script.");
  }
}
...
function createScriptPdf(r,p){
  ...
  function addPage(showHeader=true){
    if(c){drawPdfBrandLogo(x,M,H-72,250);pages.push(c.toDataURL("image/jpeg",0.94));}
    ...
    if(showHeader){
      ...
      if(!isPatientMode()&&getPsychologistName()){
